...
export default {
  ...
  data () {
    return {
      score: 0,
      rightAnswerId:  [2, 7, 12, 13, 18], // 正确答案
      scoreTips: '',
  ...
  methods: {
    handleShare () {
      document.querySelector('.share-cover').setAttribute('style', '')
    },
    handleCover () {
      document.querySelector('.share-cover').setAttribute('style', 'display:none')
    },
    // 计算分数
    computedScore () {
  ...
  created () {
    this.computedScore()
    this.getScoreTip()
    document.querySelector('body').setAttribute('style', 'background-image: url(https://cangdu.org/happyfri/static/img/4-1.jpg); padding-top:56.25px')
  }
}
</script>
...
<style lang='scss'>
  .score-contain {
    width: 454.69px;
    height: 426.56px;
    background: url(https://cangdu.org/happyfri/static/img/4-2.png) no-repeat;
