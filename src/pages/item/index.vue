    ...
    <div>
      <div class="item-back item-container-style">
        <div class="item-list-container">
           <header class="item-title">{{itemDetail[itemNum-1].topic_name}}</header>
            <ul>
              ...
              >
                <span
                  class="option-style"
                  v-bind:class='{choose: choosedNum === index}'
                >
                  {{chooseType(index)}}
                </span>
          ...
          <span class="next-item" @click='nextItem' v-if='itemNum < itemDetail.length'></span>
          <span class="submit-item" v-else @click='submitAnswer'></span>
        </div>
      </div>
  </div>
</template>
...
export default {
  ...
  created () {
    document.querySelector('body').setAttribute('style', 'background-image:url(https://cangdu.org/happyfri/static/img/1-1.jpg)')
  }
}
</script>
